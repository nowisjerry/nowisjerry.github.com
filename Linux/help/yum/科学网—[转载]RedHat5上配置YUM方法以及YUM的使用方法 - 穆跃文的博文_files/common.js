...
function ajaxpost(formid, showid, waitid, showidclass, submitbtn, recall) {
	...
	if(submitbtn) {
		if(document.getElementById(submitbtn)) {
			document.getElementById(submitbtn).disabled = true;	//add by wangmy 2012.8.21
		} else {
...
function loadcss(cssname) {
	if(!CSSLOADED[cssname]) {
		if(!document.getElementById('css_' + cssname)) {
			...
			document.getElementById('css_' + cssname).href = 'data/cache/style_' + STYLEID + '_' + cssname + '.css?' + VERHASH;
		}
		CSSLOADED[cssname] = 1;
	}
}
...
function creditShow(creditinfo, notice, basev, bk, first, creditrule) {
	...
	for(i = 1; i <= 8; i++) {
		...
		if(document.getElementById('hcredit_' + i)) {
			document.getElementById('hcredit_' + i).innerHTML = basev[i];
		}
	}
	for(i = 1; i <= 8; i++) {
		if(notice[i] != 0) {
	...
	if(!document.getElementById('creditpromptdiv')) {
		...
		document.getElementById('creditpromptdiv').innerHTML = s;
	}
	setTimeout(function () {hideMenu(1, 'prompt');document.getElementById('append_parent').removeChild(document.getElementById('ntcwin'));}, 1500);
}
...
function showColorBox(ctrlid, layer, k) {
	if(!document.getElementById(ctrlid + '_menu')) {
		...
		for(var i = 0; i < 40; i++) {
			...
			+ (typeof wysiwyg == 'undefined' ? 'seditor_insertunit(\'' + k + '\', \'[color=' + coloroptions[i] + ']\', \'[/color]\')' : (ctrlid == editorid + '_tbl_param_4' ? 'document.getElementById(\'' + ctrlid + '\').value=\'' + coloroptions[i] + '\';hideMenu(2)' : 'discuzcode(\'forecolor\', \'' + coloroptions[i] + '\')'))
...
function smilies_onload(id, smcols, seditorkey) {
	...
	if(typeof smilies_type == 'object') {
		...
		for(i in smilies_type) {
			if(smilies_type[i][0]) {
				smiliestype += '<li><a href="javascript:;" hidefocus="true" ' + (CURRENTSTYPE == i ? 'class="current"' : '') + ' id="'+seditorkey+'stype_'+i+'" onclick="smilies_switch(\'' + id + '\', \'' + smcols + '\', '+i+', 1, \'' + seditorkey + '\');if(CURRENTSTYPE) {document.getElementById(\''+seditorkey+'stype_\'+CURRENTSTYPE).className=\'\';}this.className=\'current\';CURRENTSTYPE='+i+';doane(event);">'+smilies_type[i][0]+'</a></li>';
			}
		}
		smiliestype += '</ul></div>';
...
function seditor_showimgmenu(seditorkey) {
	var imgurl = document.getElementById(seditorkey + '_image_param_1').value;
	var width = parseInt(document.getElementById(seditorkey + '_image_param_2').value);
	var height = parseInt(document.getElementById(seditorkey + '_image_param_3').value);
	var extparams = '';
	if(width || height) {
		extparams = '=' + width + ',' + height
	}
	seditor_insertunit(seditorkey, '[img' + extparams + ']' + imgurl, '[/img]', null, 1);
	document.getElementById(seditorkey + '_image_param_1').value = '';
	hideMenu();
}
function seditor_menu(seditorkey, tag) {
	var sel = false;
	if(!isUndefined(document.getElementById(seditorkey + 'message').selectionStart)) {
		sel = document.getElementById(seditorkey + 'message').selectionEnd - document.getElementById(seditorkey + 'message').selectionStart;
	} else if(document.selection && document.selection.createRange) {
		document.getElementById(seditorkey + 'message').focus();
		var sel = document.selection.createRange();
		document.getElementById(seditorkey + 'message').sel = sel;
...
function loadimgsize(imgurl, editor, p) {
	...
	s.loadCheck = function () {
		if(s.img.complete) {
			document.getElementById(editor + p + '_param_2').value = s.img.width ? s.img.width : '';
			document.getElementById(editor + p + '_param_3').value = s.img.height ? s.img.height : '';
		} else {
...
function initTab(frameId, type) {
	if (typeof document['diyform'] == 'object' || document.getElementById(frameId).className.indexOf('tab') < 0) return false;
	type = type || 'click';
...
function initSearchmenu() {
	if(!document.getElementById('sctype') || !document.getElementById('sctype_menu')) return false;
	var o = document.getElementById('sctype');
	var s = o.getAttribute('href');
	var a = document.getElementById('sctype_menu').getElementsByTagName('label');
	var r = document.getElementById('sctype_menu').getElementsByTagName('input');
	var attr = 'for';
	...
	for(var i=0; i<a.length; i++){
		...
		a[i].onclick = function(){
			...
			o.setAttribute('href', s + '?mod=' + document.getElementById(this.getAttribute(attr)).value);
		};
		r[i].onclick = function(){
			document.getElementById('sctype_menu').style.display = 'none';
		};
		if(r[i].checked) {
...
function inituserabout() {
	...
	var sideArea = document.getElementById('pp').parentNode;
	if(sideArea.className == 'sd pph') {
		document.getElementById('ppc_menu').className = 'p_pop blk';
		document.getElementById('ppc').onmouseover = function() {getAppIcon();showMenu('ppc');}
	} else {
		getAppIcon();
		document.getElementById('ppc_menu').className = 'bbs';
		document.getElementById('ppc_menu').style.display = 'block';
	}
}
function getAppIcon() {
	var appIcons = document.getElementById('ppc_menu').getElementsByTagName('img');
...
function showUser() {
	document.getElementById('newpmbox_menu').style.display = USERABOUT_BOX ? '' : 'none';
	if(USERABOUT_BOX) {
		var x = new Ajax();
...
function extstyle(css) {
	...
	document.getElementById('css_extstyle').href = css ? css + '/style.css' : STATICURL + 'image/common/extstyle_none.css';
	currentextstyle = css;
	setcookie('extstyle', css, 86400 * 30);
	if(document.getElementById('css_widthauto') && !document.getElementById('css_widthauto').disabled) {
...
function widthauto(obj) {
	if(document.getElementById('css_widthauto')) {
		CSSLOADED['widthauto'] = 1;
	}
	if(!CSSLOADED['widthauto'] || document.getElementById('css_widthauto').disabled) {
		if(!CSSLOADED['widthauto']) {
			loadcss('widthauto');
		} else {
			document.getElementById('css_widthauto').disabled = false;
		}
		HTMLNODE.className += ' widthauto';
		setcookie('widthauto', 1, 86400 * 30);
		obj.innerHTML = '切换到窄版';
	} else {
		document.getElementById('css_widthauto').disabled = true;
...
function updatesecqaa(idhash) {
	if(document.getElementById('secqaa_' + idhash)) {
		document.getElementById('secqaaverify_' + idhash).value = '';
		if(secST['qaa_' + idhash]) {
			clearTimeout(secST['qaa_' + idhash]);
		}
		document.getElementById('checksecqaaverify_' + idhash).innerHTML = '<img src="'+ IMGDIR + '/none.gif" width="16" height="16" class="vm" />';
		ajaxget('misc.php?mod=secqaa&action=update&idhash=' + idhash, 'secqaa_' + idhash, null, '', '', function() {
			secST['qaa_' + idhash] = setTimeout(function() {document.getElementById('secqaa_' + idhash).innerHTML = '<span style="cursor:pointer" class="xi2" onclick="updatesecqaa(\''+idhash+'\')">刷新验证问答</span>';}, 180000);
		});
	}
}
	...
	if(isUndefined(play)) {
		if(document.getElementById('seccode_' + idhash)) {
			...
			ajaxget('misc.php?mod=seccode&action=update&idhash=' + idhash, 'seccode_' + idhash, null, '', '', function() {
				secST['code_' + idhash] = setTimeout(function() {document.getElementById('seccode_' + idhash).innerHTML = '<span style="cursor:pointer" class="xi2" onclick="updateseccode(\''+idhash+'\')">刷新验证码</span>';}, 180000);
			});
		}
	} else {
...
function checksec(type, idhash, showmsg, recall) {
	...
	x.loading = '';
	document.getElementById('checksec' + type + 'verify_' + idhash).innerHTML = '<img src="'+ IMGDIR + '/loading.gif" width="16" height="16" class="vm" />';
...
function createPalette(colorid,id) {
	...
	if (!document.getElementById("c"+colorid+"_menu")) {
...
function cardUpdatedoing(scdoing, op) {
	if(document.getElementById(scdoing)) {
		if(!op) {
			document.getElementById('return_' + scdoing).style.display = 'none';
			document.getElementById(scdoing).style.display = '';
		} else {
			document.getElementById('return_' + scdoing).style.display = '';
			document.getElementById(scdoing).style.display = 'none';
		}
	}
}
